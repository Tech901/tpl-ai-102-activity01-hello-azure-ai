...
If you prefer to work locally, you need **Python 3.11 or newer**. Clone the repo and run:
...
> [!WARNING]
> The command to run Python varies by system. In Codespaces, `python` works. On macOS/Linux, you may need `python3`. On Windows, try `py`. Run `python --version` (or `python3 --version`) to check — if it says 3.11 or higher, you're good. Use whichever command works and substitute it throughout this guide.
...
You should see six variables (three endpoint/key pairs). `AZURE_OPENAI_DEPLOYMENT` is not set as a secret — the code defaults to `gpt-4o` automatically. If the six variables appear, skip to Step 3.
...
> [!WARNING]
> **First time editing Python?** Two things trip up almost every student:
>
> 1. **Indentation** — Python uses spaces (not curly braces) to define code blocks. When you uncomment or write code inside a function, every line must be indented to the same level as the code around it. In VS Code, select multiple lines and press `Tab` to indent or `Shift+Tab` to unindent.
> 2. **Remove the `raise NotImplementedError(...)` line** — Each function has a placeholder like `raise NotImplementedError("Configure the Azure OpenAI client")`. After you write your code for that step, **delete that line**. If you leave it in, Python will hit it and throw an error, ignoring everything you wrote above it.
...
| `python: command not found` | Python is not on your PATH, or your system uses `python3` instead. | In Codespaces, `python` works. On macOS/Linux, try `python3`. On Windows, try `py`. Use whichever command `--version` responds to and substitute it throughout. |
